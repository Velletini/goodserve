...
class SequenceGroup:
    ...
    def __init__(
        ...
        self.router_request_id = getattr(sampling_params, "router_request_id", "") if sampling_params else ""
        self.ttft_logged = False
        self.migration_state = "INITIAL"
        self._last_prediction_milestone = 0
        self.migration_triggered = False
        self.thrift_request_id = "null"
        self.destination_url = "null"
        self.deadline = sampling_params.deadline if sampling_params else None
        self.proxy_request_id = sampling_params.proxy_request_id if sampling_params else None
        self.problem = sampling_params.problem if sampling_params else None
        self.add_req_time = 0
        self.total_generated_len = sampling_params.total_generated_len
        self.current_max_tokens = sampling_params.max_tokens
        if sampling_params and getattr(sampling_params, "original_prompt_len", -1) != -1:
            self.original_prompt_len = sampling_params.original_prompt_len
        else:
            self.original_prompt_len = len(self.prompt_token_ids)
            if sampling_params:
                sampling_params.original_prompt_len = self.original_prompt_len
